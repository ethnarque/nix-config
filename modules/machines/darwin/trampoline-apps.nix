...
# Needed for indexing apps installed with home-manager
# Pulled from: https://github.com/nix-community/home-manager/issues/1341
let
  inherit (lib)
    hm
    isDarwin
  ...
  cfg = config.machines.darwin.trampolineApps;
in
{
  options.machines.darwin.trampolineApps = {
    reloadDock = mkOption {
      type = types.bool;
      default = false;
    };
  ...
  config = mkIf config.machines.darwin.enable (
      ...
      home-manager.users.${username} = {
        ...
        home.activation.trampolineApps = hm.dag.entryAfter [ "writeBoundary" ] ''
            ...
            function sync_trampolines() {
                ...
                ${optionalString cfg.reloadDock ''sync_dock "''${apps[@]}"''}
            }
